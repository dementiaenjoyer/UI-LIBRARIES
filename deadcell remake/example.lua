local library = loadstring(game:HttpGet("https://raw.githubusercontent.com/dementiaenjoyer/UI-LIBRARIES/refs/heads/main/deadcell%20remake/source.lua"))();
...
do
    ...
    local combat_page = window:new_page({name = "Combat"}); do
        local section = combat_page:new_section({
            name = "Silent"
        });
        
        local toggle = section:new_toggle({
            name = "Silent Aim",
            state = false,
            flag = "silent_aim",
        ...
        local slider = section:new_slider({
            name = "FOV Radius",
            min = 1,
            max = 420,
            default = 1,
        ...
        local color_picker = section:new_colorpicker({
            name = "FOV Color",
        ...
        local text_box = section:new_textbox({
            placeholder = "To kill",
            default = "",
            flag = "to_kill",
        ...
        section:new_button({
            ...
            confirm = true,
            callback = function(value)
                print(`Killed {library.flags.to_kill}`);
            end
        });
    
        combat_page.open(combat_page);
    end

    local settings_page = window:new_page({name = "Settings"}); do
        local theme_pickers = {};
        local accent = settings_page:new_section({
            name = "Colors",
            size = "Fill"
        });
        ...
        local menu_other = settings_page:new_section({
        	name = "Other",
        	size = "Fill",
        	side = "right"
        });
